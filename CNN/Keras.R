...
#Works forfor Windows
test1 <- as.matrix(train_dataset[,1:256])
dim(test1) <- c(1275,256,1)
x_train <-test1
y_train <- as.matrix(train_dataset[,257:266])

test2 <- as.matrix(test_dataset[,1:256])
dim(test2) <- c(318,256,1)
x_test <-test2
y_test <- as.matrix(test_dataset[,257:266])
# Works for MAC system only
